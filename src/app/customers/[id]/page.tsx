...
import { useReactToPrint } from "react-to-print"; // YAZDIRMA PAKETİ
import { PrintableStatement } from "@/components/PrintableStatement"; // YENİ BİLEŞEN
import { 
  ArrowLeft, Wallet, Printer, Plus, Minus, X, Loader2, 
  Calculator, History, Calendar
} from "lucide-react";
...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [isPrintModalOpen, setPrintModalOpen] = useState(false); // YENİ: YAZDIRMA MODALI
  const [processing, setProcessing] = useState(false);
  
  // YAZDIRMA TARİH FİLTRESİ
  // Varsayılan: Son 30 gün
  const today = new Date().toISOString().split('T')[0];
  const lastMonth = new Date();
  lastMonth.setMonth(lastMonth.getMonth() - 1);
  const [printStartDate, setPrintStartDate] = useState(lastMonth.toISOString().split('T')[0]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (setts?.store_name) setStoreName(setts.store_name);
  ...
  const openTransactionModal = (type: 'SATIS' | 'TAHSILAT', curr: 'HAS' | 'TL' | 'USD') => {
    setTrxType(type); setCurrency(curr); setAmount(""); setDescription(""); setModalOpen(true);
  };
  ...
  // --- YAZDIRMA MOTORU ---
  // --- YAZDIRMA MOTORU (DÜZELTİLDİ) ---
  const handlePrint = useReactToPrint({
    contentRef: componentRef, // 👈 content yerine contentRef, fonksiyon YOK.
    documentTitle: `Ekstre-${customer?.full_name || 'Musteri'}`,
  });

  // Yazdırılacak veriyi filtrele
  const filteredTransactions = transactions.filter(t => {
    const tDate = t.created_at.split('T')[0];
    return tDate >= printStartDate && tDate <= printEndDate;
  });
  ...
  async function handleTransaction() {
    ...
    try {
      const { error: trxError } = await supabase.from('transactions').insert({
        customer_id: id, type: trxType, product_name: currency === 'HAS' ? productType : currency,
        gram: currency === 'HAS' ? numAmount : 0, price: currency === 'TL' ? numAmount : 0,
        has_equivalent: currency === 'HAS' ? parseFloat(hasVal as string) : 0, description: autoDesc, currency: currency
      });
      if (trxError) throw trxError;
      ...
      toast.success("İşlem başarıyla işlendi.", {
        description: "WhatsApp fişi gönderilsin mi?",
        action: {
          label: "WhatsApp Gönder 📲",
          onClick: () => {
            if (!_phone || _phone.length < 10) { toast.error("Telefon no yok."); return; }
      ...
      setModalOpen(false);
      fetchData(); 
    } catch (error: any) { toast.error("Hata:", { description: error.message }); } 
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 shadow-sm">
          ...
          <div className="flex gap-3">
             {/* YAZDIR BUTONU ARTIK ÇALIŞIYOR */}
             <button onClick={() => setPrintModalOpen(true)} className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg font-bold transition">
        ...
        <div className="flex-1 overflow-y-auto p-8">
          ...
          <div style={{ display: "none" }}>
            <PrintableStatement 
                ref={componentRef} 
                customer={customer} 
                transactions={filteredTransactions} // Filtrelenmiş veri gidiyor
      ...
      {/* --- YAZDIRMA MODALI (YENİ) --- */}
      {isPrintModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-lg flex items-center gap-2"><Printer size={20} /> Ekstre Yazdır</h3>
                    <button onClick={() => setPrintModalOpen(false)}><X className="text-slate-400" /></button>
                </div>
                <div className="space-y-4">
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input type="date" className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 pl-10 text-sm" value={printStartDate} onChange={(e) => setPrintStartDate(e.target.value)} />
                        </div>
                    </div>
                    <div>
                        <label className="text-xs font-bold text-slate-500 mb-1 block">Bitiş Tarihi</label>
                        <div className="relative">
                            <Calendar className="absolute left-3 top-2.5 text-slate-400" size={16}/>
                            <input type="date" className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 pl-10 text-sm" value={printEndDate} onChange={(e) => setPrintEndDate(e.target.value)} />
                        </div>
                    </div>
                    
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 text-xs text-slate-500">
                        Seçilen tarihler arasında toplam <b>{filteredTransactions.length}</b> işlem bulundu.
                    </div>

                    <button onClick={handlePrint} className="w-full bg-slate-800 hover:bg-slate-900 text-white py-3 rounded-xl font-bold transition flex justify-center gap-2">
